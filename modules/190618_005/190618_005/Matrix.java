...
public class Matrix {
	...
	// Map
	public Matrix map(UnivariateFunction<Double> fn) {
		int m = this.data.length;
		int n = this.data[0].length;
		double[][] B = new double[m][n];
		for(int i=0; i<m; i++)
	...
	public Vector applyRowVector(MultivariateFunction fn) {
		int m = this.data.length;
		double[] w = new double[m];
		for(int i=0; i<m; i++)
			w[i] = fn.value(this.getRowVector(i));
		return new Vector(w);
	}
	
	// Apply Function to Column Vector
	public Vector applyColumnVector(MultivariateFunction fn) {
		int n = this.data[0].length;
		double[] w = new double[n];
		for(int j=0; j<n; j++)
			w[j] = fn.value(this.getColumnVector(j));
		return new Vector(w);
	}
